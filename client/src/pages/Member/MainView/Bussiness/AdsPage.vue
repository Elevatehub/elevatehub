      ...
      <v-col cols="12" style="align-items: center; text-align: center;">
        <form v-on:submit="onSearch">
          <v-row style="align-items: center;">
            <v-col>
              <InputComponent label="Pricing Strategy" placeholder="Tulis disini..." v-model="keyword"
                :required="content.length === 0" variant="outlined" />
              <br />
              <InputComponent label="Sektor Bisnis" placeholder="Tulis disini..." v-model="keyword"
                :required="content.length === 0" variant="outlined" />
            </v-col>
...
export default {
  ...
  methods: {
    ...
    async onSearch(e) {
      this.isLoading = true
      e.preventDefault()
      const payload = `Saya memiliki product ${e.target[0].value},  sebagai seorang ads analist strategy dapatkan anda membuatkan Comparison’s Google Ads headlines, Google Ads descriptions, Google Search Campaign Type( Brand- Generic or Comparison), Target Keywords, Country and CTA untuk produk saya ${e.target[1].value}`
      try {

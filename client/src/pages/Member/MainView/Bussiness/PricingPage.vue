      ...
      <v-col cols="12" style="align-items: center; text-align: center;">
        <form v-on:submit="onSearch">
          <v-row style="align-items: center;">
            <v-col>
              <InputComponent label="Nama Product" placeholder="Tulis disini..." v-model="keyword"
...
export default {
  ...
  methods: {
    ...
    async onSearch(e) {
      this.isLoading = true
      e.preventDefault()
      const payload = `Sebagai seorang analist harga, dapatkan anda membandingkan harga layanan dari produk-produk kompetitor ${e.target[0].value} yang bergerak di sektor ${e.target[1].value}`
      try {
